...
void loop(void) 
{
  ...
  success = NfcPn532.readPassiveTargetID(PN532_MIFARE_ISO14443A, uid, &uidLength);
  ...
  if (success) 
  {
    ...
    NfcPn532.PrintHex(uid, uidLength);
    Serial.println("");
    ...
    if (uidLength == 7)
    {
      ...
      {
        success = NfcPn532.ntag2xx_ReadPage(i, data);
